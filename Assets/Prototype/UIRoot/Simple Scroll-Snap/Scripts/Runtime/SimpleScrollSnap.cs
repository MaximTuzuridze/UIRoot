...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    {
        #region Fields
        [Header("Чи варто робити налаштування при старті")]
        ...
        public UIElementMenuButton[] Buttons { get; set; }
        ...
        {
            Debug.Log("OnEndDrag " + scrollRect.velocity.x);
            dragging = false;
        ...
        private void Setup()
        {
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                {
                    Debug.Log("Ініціалізація Content");
            ...
            if (pagination != null)
            {
                Buttons = pagination.GetComponentsInChildren<UIElementMenuButton>();
                for (int i = 0; i < Buttons.Length; i++)
                {
                    if (Buttons[i] != null)
                    {
                        Buttons[i].Focus(i == startingPanel);
                        int panelNum = i;
                    }
                }
            }
        ...
        private void SelectTargetPanel()
        {
            ...
            Debug.Log(releaseSpeed +" <= "+  minimumSwipeSpeed);
            ...
            if (snapTarget == SnapTarget.Nearest || releaseSpeed <= minimumSwipeSpeed)
            {
                ...
                if (releaseDirection == Direction.Left)// не важливо настільки ти зробив довгий свайп йде до кінця
                {
                    GoToPanel(CurrentPanel != ( NumberOfPanels - 1)? CurrentPanel + 1 : CurrentPanel);
                }
                else
                {
                    GoToPanel(CurrentPanel != 0 ? CurrentPanel - 1 : CurrentPanel);
                }
            }
        ...
        private void SnapToTargetPanel()
        {
            ...
            {
                if (DisplacementFromCenter(TargetPanel).magnitude < (Viewport.rect.width / 10f) || true)
                {
        ...
        public void GoToPanel(int panelNumber)
        {
            ...
            if (pagination != null)
            {
                for (int i = 0; i < Buttons.Length; i++)
                {
                    if (Buttons[i] != null)
                    {
                        Buttons[i].Focus(i == TargetPanel);
                    }
                }
                var bottomPanel = UIRoot.Instance.FindMenu("Menu.Main").FindWindowElement("PanelBottom") as UIElementMainBottomPanel;
                bottomPanel.ReInitFitter();
            }
        ...
        {
            Debug.Log("NearestPanel: " + NearestPanel);
            NearestPanel = DetermineNearestPanel();
            Debug.Log("NearestPanel2: " + NearestPanel);
